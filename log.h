...
#define log(_log_level_, ...) Log::GetInstance()->AddLog(_log_level_,__FILE__,__LINE__,__FUNCSIG__,__VA_ARGS__)
	class Log
	{
	public:
		...
		bool StartLog();
		void StopLog();
		void AddLog(LOG_LEVEL _log_level_, const char* _file_, size_t _line_num_, const char* _func_sig_, const char* psz_fmt_, ...);
	private:
		Log() = default;
		~Log();
		...
		void ThreadFunc();
	private:
		std::string _level_str[3]{ "INFO","WARNING","ERROR" };
		static Log* _ptr_log;
		std::mutex _mutex;
